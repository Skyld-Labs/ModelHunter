...
class ModelHunter:
    ...
    def __init__(self, apkpath, config, args):
        ...
        self.setup_database()
        self.setup_framework_database()
        self.setup_database_view()

        # Add test app to database
        try:
            self._cur.execute("INSERT or IGNORE INTO application(package_id) VALUES(?)", [self._pkgname])
    ...
    def setup_database(self):
        ...
        CREATE TABLE IF NOT EXISTS "hash" (
            ...
            PRIMARY KEY("id_hash" AUTOINCREMENT)
        );
        '''
        ...
        CREATE TABLE IF NOT EXISTS "framework" (
            ...
            "framework_name"	INTEGER NOT NULL UNIQUE,
            "is_open_source" TEXT,
            PRIMARY KEY("id_framework" AUTOINCREMENT)
        );
        '''

        app_framework_table = '''
        CREATE TABLE IF NOT EXISTS "app_framework" (
            "id_app_framework"	INTEGER NOT NULL,
            "id_app"	INTEGER NOT NULL,
            "id_framework" INTEGER,
            PRIMARY KEY("id_app_framework" AUTOINCREMENT)
        );
        '''

        hash_framework_table = '''
        CREATE TABLE IF NOT EXISTS "hash_framework" (
            "id_hash_framework"	INTEGER NOT NULL,
            "id_hash"	INTEGER NOT NULL,
            "id_framework" INTEGER,
            PRIMARY KEY("id_hash_framework" AUTOINCREMENT)
        );
        '''


        self._connect = sqlite3.connect("model_stat.db")
        self._cur = self._connect.cursor()

        self._cur.execute(hash_table)
        self._connect.commit()

        self._cur.execute(model_table)
        self._connect.commit()

        self._cur.execute(application_table)
        self._connect.commit()

        self._cur.execute(framework_table)
        self._connect.commit()

        self._cur.execute(app_framework_table)
        self._connect.commit()

        self._cur.execute(hash_framework_table)
        self._connect.commit()

        pass

    def setup_database_view(self):
        all_essential_information_view = """
        CREATE VIEW IF NOT EXISTS essential_information AS SELECT all_info.package_id, all_info.model_name, all_info.md5_hash, all_info.entropy, all_info.file_size, all_info.probability, count_model, concat_framework FROM 
        (SELECT * FROM model NATURAL JOIN hash NATURAL JOIN application) AS all_info JOIN 
        (SELECT md5_hash,count(id_model) as count_model FROM model 
            LEFT JOIN hash ON model.id_hash=hash.id_hash GROUP BY hash.md5_hash) 
            AS count_model ON count_model.md5_hash=all_info.md5_hash NATURAL JOIN 
            (SELECT package_id, group_concat(framework_name) as concat_framework 
                FROM (application LEFT JOIN app_framework ON application.id_app=app_framework.id_app) 
                    LEFT JOIN framework ON app_framework.id_framework=framework.id_framework GROUP BY package_id)
                    AS concat_app_framework ORDER BY all_info.package_id;
        """
    ...
    def setup_framework_database(self):
        ...
        for fw in frameworklist:
            is_open_source = self._config.get(fw.strip(),"is_open_source")

            try:
                self._cur.execute("INSERT or IGNORE INTO framework(framework_name, is_open_source) VALUES(?, ?)", [fw, is_open_source])
                self._connect.commit()
            except Exception as e:
                print(e)
    ...
    def header_analyzer(self, assets_path, filelist):
        res = []
        for f in filelist:
            with open((assets_path + '/' + f), 'rb') as fileread:
                for fmn in self._FRAMEWORK_MAGIC_NUMBER:
                    if(self._FRAMEWORK_MAGIC_NUMBER[fmn] in fileread.read(10)):
                        res.append(f)
        return res
    ...
    def header_filter(self, assets_path, filelist):
        ...
        for f in filelist:
            with open((assets_path + '/' + f), 'rb') as fileread:
                for fnh in filter_not_header:
                    if(filter_not_header[fnh] in fileread.read(10)):
                        filelist.remove(f)
                        break
        print(filelist)
        return filelist

    def is_header_model(self, header):
        for fmn in self._FRAMEWORK_MAGIC_NUMBER:
            if (self._FRAMEWORK_MAGIC_NUMBER[fmn] in header):
                return True
        return False
    ...
    def extract_model_path(self):
        ...
        for model_detected in res_hd:
            if model_detected not in res:
                print("NEW MODEL NOT DETECT BY DEFAULT METHOD :", model_detected)
                with open('/tmp/new_result', 'a') as new_result:
                    new_result.write(model_detected)
        res = list(set().union(res, res_hd))
        ...
        for f in res:
            ...
            # Create probability from unlikely, likely, sure
            # 
            probability_mask = 0
            if (f in res_kw):
                probability_mask |= 1 << 0
            if (self._guess_fw != None and len(self._guess_fw) > 0):
                probability_mask |= 1 << 1
            if (self.is_header_model(header)):
                probability_mask |= 1 << 2
            if (f in res_suf ):
            ...
            # Get entropy
            try:
                with open(ff, 'rb') as model_read:
                    ent = str(self.entropy(model_read.read()))
                    print(ent)
            except:
                ent = ""
            ...
            try:
                ...
                id_app = self._cur.execute("SELECT id_app FROM application WHERE package_id=?", [self._pkgname])
    ...
    def guess_ml_framework_by_magic_str(self, lib_files):
        ...
        for fw in frameworklist:
            ...
            if flag:
                # Add framework in database 
                try:
                    self._cur.execute("SELECT id_framework FROM framework WHERE framework_name=?", [fw])
                    id_framework = self._cur.fetchone()

                    self._cur.execute("SELECT id_app FROM application WHERE package_id=?", [self._pkgname])
                    id_app = self._cur.fetchone()
                    
                    if id_framework and id_app:   
                        self._cur.execute("INSERT or IGNORE INTO app_framework(id_app,id_framework) VALUES(?, ?)", [id_app[0], id_framework[0]])
                        self._connect.commit()
                except Exception as e:
                    print(e)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog='ModelHunter')
    parser.add_argument('apkpath',
            help = 'path to apk file or directory')
    parser.add_argument('-c', '--config-file', default = 'modelhunter.config',
    ...
    parser.add_argument('-l', '--log-file', action='store_true',
            help = 'store log in modelhunter.log(default to stdout)')
    ...
    if args.log_file is True:
        if args.verbose is True:
            logging.basicConfig(filename='modelhunter.log', level=logging.DEBUG)
        else:
            logging.basicConfig(filename='modelhunter.log', level=logging.INFO)
    else:
        if args.verbose is True:
    ...
    if os.path.isfile(args.apkpath):
        ...
        # Global Progress Bar
        bar = progressbar.ProgressBar(maxval=len(jobs)).start()
        # create workers 
        workers = []
        mgr = Manager()
        ns = mgr.Namespace()
